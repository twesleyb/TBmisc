...
# A command line function to download a file using R's download.file.

#url <- "https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6338729/pdf/41467_2018_Article_8191.pdf"
...
parser$add_argument("url", nargs=1, help="File to be displayed")
parser$add_argument("destfile", nargs=2, help="File to be displayed")

# Add optional arguments.
parser$add_argument("-M", "--method", action="store_true", default="auto")
parser$add_argument("-q", "--quiet", action="store_true", default=FALSE)
parser$add_argument("-m", "--mode", action="store_true", default="w")
parser$add_argument("-c", "--cacheOK", action="store_true", default=TRUE)
parser$add_argument("-h", "--headers", action="store_true", default=NULL)

rdoc <- get_rdoc("download.file","utils")

rdoc <- get_rdoc("download.file","utils",arg="mode")
...
get_rdoc <- function(fun,pkg=NULL,arg=NULL,...){
	...
	drop_empty_str <- function(string) { string[string!=""] }
	...
	get_keywords <- function(latex){
		# Get keywords.
		keywords <- drop_empty_str({
			unlist(strsplit(latex[grep("keyword\\{",latex)],"\\{|\\}"))[-c(1)]
		})
